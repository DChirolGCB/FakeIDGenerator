...
class DeepDream(object):
    ...
    @staticmethod
    def _Kfunc(*argtypes):
        '''Helper that transforms K-graph generating function into a regular one.
        See `_resize` function below.
        '''
        placeholders = list(map(K.placeholder, argtypes))
    ...
    def _base_resize(img, size):
        '''Helper function that uses K to resize an image'''
        img = K.expand_dims(img, 0)
        return K.image.resize_bilinear(img, size)[0, :, :, :]
    ...
    def __init__(self):
        ...
        self._graph = K.Graph()
        self._session = K.InteractiveSession(graph=self._graph)
        self._resize = self._Kfunc(np.float32, np.int32)(self._base_resize)
        with K.gfile.FastGFile(DeepDream._MODEL_FILENAME, 'rb') as f:
            graph_def = K.GraphDef()
            graph_def.ParseFromString(f.read())
        self._t_input = K.placeholder(np.float32, name='input')  # define the input tensor
        imagenet_mean = 117.0
        t_preprocessed = K.expand_dims(self._t_input - imagenet_mean, 0)
        K.import_graph_def(graph_def, {'input': t_preprocessed})

        self.t_obj = self.T('mixed4d_3x3_bottleneck_pre_relu')[:, :, :, 139]
        # self.t_obj = K.square(self.T('mixed4c'))
    ...
    def process_frame(self, frame, iter_n=10, step=1.5, octave_n=4, octave_scale=1.4):
        t_score = K.reduce_mean(self.t_obj)  # defining the optimization objective
        t_grad = K.gradients(t_score, self._t_input)[0]  # behold the power of automatic differentiation!
